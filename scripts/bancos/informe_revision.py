#!/usr/bin/env python3
"""Excel de revision y borrador de correo para quien va a importar el fichero.
...
def borrador_correo(clasificados, cuentas, verificacion, asientos: int) -> str:
    puente = cuentas.get("puente", "5550000")
    vivos = [c for c in clasificados if not c.get("contabilizado_en_otro")]
    en_puente = [c for c in vivos if c["contrapartida"] == puente]
    pct = len(en_puente) / len(vivos) * 100 if vivos else 0
    nuevas = sorted({c.strip() for a in (verificacion or {}).get("avisos", [])
                     if "dar de alta" in a
                     for c in a.split(":")[-1].split(",")}) if verificacion else []
    cuadra = (verificacion or {}).get("correcto")

    return f"""Asunto: XDIARIO para importar — {asientos} asientos

Hola:

Te paso el fichero de bancos para importar en ContaPlus.

QUÉ SE ENTREGA
- XDIARIO.DBF con {asientos} asientos ({asientos * 2} apuntes).
- Excel de revisión con el cuadre, lo que hay que mirar y la traza completa.

CUADRE
{'Cuadra al céntimo con los extractos y con el cierre del ejercicio anterior.'
 if cuadra else 'ATENCIÓN: el cuadre NO sale. Mira la hoja Resumen antes de importar.'}

SUBCUENTAS QUE HAY QUE DAR DE ALTA ANTES DE IMPORTAR
{chr(10).join('- ' + c for c in nuevas) if nuevas
 else '- Ninguna: todas las cuentas usadas ya existen en el plan.'}

QUEDA EN LA CUENTA PUENTE {puente}
{len(en_puente)} movimientos, un {pct:.1f} % del total. Están en la hoja «A revisar»,
con el texto original del extracto al lado para poder localizarlos. Los motivos
principales están desglosados en «Notas y avisos».

He preferido dejarlos en la puente antes que imputarlos sin respaldo en el
histórico del cliente.

DECISIONES DE CRITERIO PENDIENTES
- ¿Las compras con tarjeta van a compras o a gastos? Cargarlas contra la cuenta
  del proveedor descuadra su saldo si no hay factura detrás.
- Confirmar el periodo de las liquidaciones de Seguridad Social.
- Revisar los cobros imputados contra cuentas de proveedor.
...
def main() -> int:
    ...
    ap.add_argument("--correo", type=Path)
    ...
    if args.correo:
        args.correo.parent.mkdir(parents=True, exist_ok=True)
        args.correo.write_text(
            borrador_correo(clasificados, cuentas, verificacion, asientos), encoding="utf-8")
        print(f"Borrador de correo: {args.correo}")
